...
- **OR test is currently muted and has only skipped runs**  
  (is_muted = 1, skip > 0, pass = 0, fail = 0, mute = 0)
...
- If a test is muted and had only skipped runs in 7 days — it will be removed from mute.

## 🔄 Automated Workflow

### Automatic muted_ya.txt Updates
The `.github/workflows/update_muted_ya.yml` workflow automatically:
- Runs every hour from 3:00 to 21:00 UTC
...
### 🗑️ [to_delete.txt](mute_update/to_delete.txt)
**Content:** Tests to remove from mute  
**Rules:** No runs in 7 days **OR** muted + only skipped in 7 days  
**Usage:** Main file for removal from mute

### 🔍 [muted_ya.txt](.github/config/muted_ya.txt)
**Content:** Current mute rules stored in the repository  
**Usage:** Baseline mute file used by the workflow

### 🔍 [new_muted_ya.txt](mute_update/new_muted_ya.txt)
**Content:** Final mute file generated by `create_new_muted_ya.py`  
**Usage:** Used to update `.github/config/muted_ya.txt` in PR

### 🔍 [muted_ya_changes.txt](mute_update/muted_ya_changes.txt)
**Content:** Consolidated change list with prefixes  
**Prefixes:** `+++` add mute, `---` unmute, `xxx` delete from mute  
**Usage:** Human-readable delta report
