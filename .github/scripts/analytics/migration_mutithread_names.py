    ...
    def callee(session):
        session.execute_scheme(f"""
            CREATE table IF NOT EXISTS`{table_path}` (
                ...
                PRIMARY KEY (suite_folder,test_name, branch, build_type ,full_name,run_timestamp)
            )
...
def process_timestamp(driver, table_client, test, date):
    query_get_runs = f"""
    select * from `test_results/test_runs_column`
    where test_name = '{test}' 
    and run_timestamp >= cast('{date}' as Timestamp)
    """
    ...
    print(f'runs for test {test}: {end_time - start_time}')
    ...
    print('Upserting runs for test:', test)
    ...
    print('History updated for test:',test)
...
def main():
    ...
    with ydb.Driver(
        ...
        last_date_query = f"""select distinct test_name from `test_results/test_runs_column`
        ...
        while True:
            try:
                result = next(it)
                test_names = test_names + result.result_set.rows
            except StopIteration:
                break

        end_time = time.time()
        print(f"Transaction 'geting timestamp list from runs' duration: {end_time - start_time}")
        print(f'Count of tests: {len(test_names)}')

        # Create and start threads
        threads = []
        active_threads = 0
        for test_item in test_names:
            test= test_item['test_name']
            print(f'upload {test}')
            ...
            thread = threading.Thread(target=process_timestamp, args=(driver, table_client, test, last_date))
