...
def main():
    ...
    with ydb.Driver(
        ...
        last_date_query = f"select max(run_timestamp) as last_run_timestamp from `test_results/test_runs`"
        ...
        if results[0] and results[0].get( 'last_run_timestamp', default_start_date) is not None and results[0].get( 'last_run_timestamp', default_start_date) > default_start_date:
            last_datetime = results[0].get(
                'last_run_timestamp', default_start_date)
        ...
        date_list = [today - datetime.timedelta(days=x) for x in range((today - last_datetime.date()).days+1)]
        ...
        for date in sorted(date_list):
            ...
            thread = threading.Thread(target=process_timestamp, args=(driver, table_client, date))
